    ...
    <FunnelFrame currentStep={currentStep} onNext={onNext} onPrev={onPrev}>
      <section className="bg-bg px-[clamp(20px,5.5vw,44px)] pt-2">
        ...
        <div className="space-y-5">
          <section className="rounded-[22px] bg-white px-[clamp(18px,4.5vw,28px)] py-[clamp(24px,6vw,32px)] shadow-card">
            <label
              htmlFor="avoid-hangul"
              className="block text-[clamp(16px,4.4vw,19px)] font-bold leading-tight tracking-normal text-[#15386D]"
            >
              1. 회피 글자 혹은 이름
              <span className="ml-1 text-[clamp(13px,3.4vw,15px)] font-semibold text-[#687896]">
                (없으면 비워주세요)
              </span>
            </label>
            <p className="mt-5 text-[clamp(13px,3.6vw,16px)] font-semibold leading-normal tracking-normal text-[#687896]">
            ...
            <div className="relative mt-6">
              <input
                ...
                className="h-[clamp(52px,14vw,60px)] w-full rounded-lg border-2 border-primary-light bg-white px-4 pr-16 text-[clamp(15px,4.4vw,19px)] font-semibold leading-none tracking-normal text-ink outline-none transition placeholder:text-primary-muted focus:border-primary focus:shadow-[0_0_0_4px_rgba(124,111,205,0.12)]"
              />
              <span className="pointer-events-none absolute right-4 top-1/2 -translate-y-1/2 text-[clamp(14px,4vw,18px)] font-bold leading-none tracking-normal text-primary">
            ...
            <div
              ...
              className="mt-5 flex items-center gap-1 text-[13px] font-semibold leading-normal tracking-normal text-[#687896]"
            >
              <Info
                aria-hidden="true"
                className="mt-[2.4px] size-[15px] shrink-0 self-start text-[#687896]"
                strokeWidth={2.2}
              />
          ...
          <section className="rounded-[22px] bg-white px-[clamp(18px,4.5vw,28px)] py-[clamp(24px,6vw,32px)] shadow-card">
            ...
            <div className="relative mt-6">
              <span className="pointer-events-none absolute left-3 top-1/2 flex size-[clamp(34px,9vw,40px)] -translate-y-1/2 items-center justify-center rounded-[10px] bg-primary-pale text-[clamp(17px,5vw,21px)] font-bold leading-none tracking-normal text-primary">
                漢
              </span>
              <input
                id="avoid-hanja"
                ...
                placeholder="예) 집 가, 家, 가"
                aria-label="회피 한자 검색"
                className="h-[clamp(52px,14vw,60px)] w-full rounded-lg border border-divider bg-white py-0 pl-[clamp(52px,14vw,70px)] pr-12 text-[clamp(15px,4.2vw,18px)] font-bold leading-none tracking-normal text-ink outline-none transition placeholder:text-ink-light focus:border-primary focus:shadow-[0_0_0_4px_rgba(124,111,205,0.12)]"
              />
              <Search
                aria-hidden="true"
                className="pointer-events-none absolute right-4 top-1/2 size-5 -translate-y-1/2 text-primary-muted"
                strokeWidth={2.4}
              />

              {isDropdownOpen && hanjaQuery ? (
                <div className="absolute left-0 right-0 top-full z-20 mt-2 max-h-[320px] overflow-y-auto rounded-lg border border-divider bg-white shadow-card">
                    ...
                    ? searchState.options.map((option) => {
                        ...
                        return (
                          <button
                            ...
                            className={[
                              "flex min-h-14 w-full items-center justify-between gap-3 px-4 text-left transition",
                              isSelected
                          ...
                          >
                            <span className="flex min-w-0 items-center gap-3">
                              <span className="text-[clamp(22px,6vw,26px)] font-bold leading-none tracking-normal">
                                {option.hanja}
                              </span>
                              <span className="min-w-0 truncate text-[clamp(12px,3.4vw,15px)] font-semibold leading-tight tracking-normal text-[#687896]">
              ...
              <div className="mt-4 flex flex-wrap gap-2">
                {selectedHanja.map((option) => (
                  <button
                    ...
                    className="flex min-h-[clamp(32px,9vw,40px)] items-center gap-1.5 rounded-full border border-primary-pale bg-[#FBFAFF] px-[clamp(8px,2.5vw,12px)] text-[clamp(11px,3.2vw,15px)] font-bold leading-none tracking-normal text-primary"
                  >
                    <span className="text-[clamp(15px,4.2vw,19px)]">
                      {option.hanja}
                    </span>
                    {getHanjaLabel(option)}
                    <X
                      aria-hidden="true"
                      className="size-[clamp(12px,3.5vw,16px)]"
                      strokeWidth={2.6}
                    />
                  </button>
                ))}
            ...
            <div className="mt-7 flex items-start gap-2 rounded-[14px] bg-[#F5EFFF] p-[clamp(12px,3.5vw,16px)] text-primary">
              <Lightbulb
                aria-hidden="true"
                className="mt-0.5 size-[clamp(20px,5.5vw,28px)] shrink-0"
                strokeWidth={2.2}
              />
              <div>
                <p className="text-[clamp(14px,4vw,16px)] font-bold leading-normal tracking-normal text-primary">
                  알려드려요
                </p>
                <ul className="mt-2 list-disc space-y-1 text-[clamp(12px,3.4vw,14px)] font-semibold leading-normal tracking-normal text-[#687896] marker:text-primary">
