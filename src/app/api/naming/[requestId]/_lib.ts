...
export function calcScore(
  ...
  // 흉(凶)은 화면에 그대로 노출되므로, 점수도 그 무게를 반영해야
  // "종합 1등인데 凶이 보이는" 모순이 생기지 않는다.
  //
  // 예전에는 총격 흉이 -5뿐이라 부족 오행 두 글자 보완(+20)이 흉을 덮었고,
  // 원형이정의 흉은 mixed(中)와 아무 차이가 없었다. 그 결과 총격이 흉인
  // 이름이 무결점 이름과 동점 1위가 되는 사례가 실제로 나왔다.
  //
  // 계수는 실데이터(유료 완주 10건, 이름 200개) 재정렬 시뮬레이션으로 정했다:
  // -12/-20 적용 시 1위에 흉 포함 10% → 0%, 상위 5위 내 흉 이름 29 → 17개.
  // 남는 17개는 풀에 무결점 이름이 5개 미만인 요청들 — 그 경우 흉을 숨기지
  // 않고 정직하게 노출하는 것이 의도된 동작이다.
  if (grids.총격.luck === "bad") score -= 20;
  for (const key of ["원격", "형격", "이격", "정격"] as const) {
    if (grids[key].luck === "bad") score -= 12;
  }

  // 흉이 겹치면 이론상 음수까지 내려간다(goodCount 하한 2 기준 최저 -14).
  // 화면의 별점·라벨이 0~100을 전제하므로 하한을 둔다.
